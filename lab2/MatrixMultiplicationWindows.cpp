...
struct ThreadArgs {
    ...
    class MatrixMultiplier* multiplier;
};
        ...
        for(int rowIndexA = 0; rowIndexA < numberOFBlocks; rowIndexA++) {
            for(int colIndexA = 0; colIndexA < numberOFBlocks; colIndexA++) {
                for(int colIndexB = 0; colIndexB < numberOFBlocks; colIndexB++) {
                    ...
                    args->blockRowA = rowIndexA;
                    args->blockColA = colIndexA;
                    args->blockRowB = rowIndexB;
                    args->blockColB = colIndexB;
                    args->multiplier = this;
...
DWORD WINAPI ThreadWrapper(LPVOID lpParam) {
    ThreadArgs* data = static_cast<ThreadArgs*>(lpParam);
    
    data->multiplier->getAndMultiplyBlocks(
        data->blockRowA, 
        data->blockColA, 
        data->blockRowB, 
        data->blockColB
